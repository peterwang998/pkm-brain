...
def test_acceptance_report_checks_single_configured_peer(tmp_path: Path) -> None:
    ...
    report = run_acceptance_report(primary, transport=LocalRsyncTransport(remote_home=secondary_home))
...
def test_acceptance_report_can_execute_sync_and_verify_retrieval(tmp_path: Path) -> None:
    primary, secondary_home = primary_with_secondary(tmp_path)
    phrase = "secondary acceptance unique token"
    write_outbox_file(secondary_home, "agent_logs/codex/session.md", agent_markdown("session", phrase))
