...
def test_freezer_builds_owner_only_v3_artifacts_without_external_calls(
    ...
    for artifact in (manifest_path, gold_path, marker_path):
...
def test_freezer_rejects_gold_absent_from_production_frontier(tmp_path: Path) -> None:
    fixture = _fixture()
    fixture["cases"][0]["members"][0]["value"] = "2027-10-31"

    with pytest.raises(
        builder.PublicChallengeFreezerError,
        match="gold member is absent",
    ):
        _freeze(tmp_path, fixture)

    assert not (tmp_path / "frozen" / "challenge.json").exists()
    assert not (tmp_path / "frozen" / "gold.json").exists()


def test_freezer_rejects_required_canonical_subject_absent_from_frontier(
    tmp_path: Path,
) -> None:
    fixture = _fixture()
