...
`retrieve_context` uses bounded retrieval so noisy sources do not consume the whole agent context. The default mode uses an 8,000-token context budget, source-specific caps, serialized packet-size trimming, and excerpts oversized chunks. Agent session logs are compressed more aggressively than curated notes or meeting sources. The MCP tool intentionally keeps a simple `task`/`project` surface and uses the default bounded mode. The CLI and service layer also support `--mode compact`, `--mode broad`, and `--mode inspect` for manual or scripted use. Non-debug packets keep compact scores, IDs, excerpts, and provenance; debug/inspect paths retain ranking diagnostics such as `selection_reasons`, suppression reasons, matched terms, and fanout details.
