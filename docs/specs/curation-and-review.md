...
**Last verified:** 2026-07-31 against the temporal-cognition working tree; current-run audit is isolated from bounded weekly historical review, and semantic human resolutions survive regenerated action IDs
...
Audit has two deliberately separate time horizons. The nightly curation cycle audits the exact action IDs returned by that cycle's extraction, gardener, and synthesis stages. This includes an older open action reused and applied during the current cycle without rewriting its original `run_id`. An empty current-cycle action list is an explicit zero-action audit and must never fall through to the global historical backlog. Historical coverage is a bounded weekly maintenance pass, not hidden nightly work: it selects at most five unresolved actions, takes no more than one representative from the same source/evidence group, and prioritizes direct contradictions, still-current targets, retrieval exposure/recency, and risk. Selection reads a deterministic window of at most 512 unaudited applied actions in 64-row keyset batches ordered newest-first by `(COALESCE(applied_at, created_at), id)`, then applies semantic priority inside that window. The durable scan state retains both its start-after cursor and the exact bounded action-ID membership whenever the window yields a cohort, so newer or backdated inserts cannot displace unselected eligible rows. Only a successful scan that finds no eligible cohort advances past the original window tail; reaching the historical tail resets the next scan to the head. That bounded cycle prevents manually resolved or policy-unsampled rows near the head from starving older actions. A partial-capacity retry remains pending with the exact persisted cohort until all still-eligible members are audited or otherwise terminal. Fact-priority enrichment is also bounded to 4,096 fairly interleaved target IDs, loaded in SQLite-safe 400-bind chunks. Historical findings carry an explicit `weekly_historical` origin, do not demote the active policy from old evidence, and are admitted as capacity-bounded maintenance unless a current direct contradiction makes them a hard boundary.
...
Human review is durable at the semantic state level rather than the generated Queue/action ID level. `review_resolutions` stores the action family, an exact evidence-and-temporal state fingerprint, a portable evidence key, source group, disposition, decision origin, and revocation state. For facts, identity includes the normalized statement, normalized entity attribution, source IDs/spans and quote, predicate-validity fields, and flat or nested event-time fields; generated IDs, confidence, routing, timestamps, and other process noise do not create a new issue. Portable identity excludes installation-local entity IDs, while exact local state includes an explicitly supplied ID.

- An explicit Keep/Reject audit decision, a human-decided linked question, or a user-confirmed fact suppresses an exact regenerated finding after reindexing or restart.
- A multi-fact comparison records one semantic decision per exact fact state: selected alternatives are Keep, losing alternatives are Reject, and exact duplicate copies resolve once with Keep taking precedence.
- `route` and `new_page` accept the semantic fact and therefore record one Keep for the applied routed replacement, not a Reject for the pre-route candidate. Because routing is intentionally absent from fact review identity, regenerated exact copies close as already kept; the physical superseded proposal can remain rejected without reversing the human truth decision.
- Evidence, predicate validity, or event-time changes create a new state fingerprint and reopen review. A broad source group may bound weekly sampling but never transfers a human decision to a materially different issue.
- Exact candidates suppressed after a decision append their full post-state fingerprint and prior review state to that decision's system-owned rollback journal. The journal may grow only by authenticated suffixes; Undo validates and restores the complete action/question closure set before revoking the resolution.
- Undo handles bind both the exact post-decision database state and their canonical restore payload. Stale, replayed, drifted, or modified handles fail without mutating actions or creating residue. Guard validation, every database inverse/reopen, and every associated resolution revocation share one write-locked transaction; rebuildable Wiki/memory projections run after commit and report an explicit repair state if they cannot be refreshed safely. Native managed-page curation compensates exact file and page-index state when a downstream step throws; a foreign or interrupted curator that already changed a page must leave a durable before/after snapshot for the sealed Undo handle, or compensate the change immediately if that snapshot cannot be persisted.
- Keep Applied Fact confirms the current fact, records `sampled_ok`, and writes its semantic Keep receipt in one transaction. A concurrent loser or interrupted write therefore cannot roll back another reviewer's confirmation or leave confirmation without the matching audit decision.
- A question decision and its resolution-ledger write form one compensated operation: failure restores the question and prior action state and reverts replacement mutations.
- Migration backfill is idempotent for existing confirmed facts, human-decided questions, per-fact alternative selections, and explicit audit decisions. Schema 28 repairs incomplete or misclassified schema-27 derived question rows; schema 29 reruns that backfill after the entity-identity change, records best-effort receipts for exact historical mention attribution recoverable from source actions or `fact_entities`, and preserves reconstructable direct Queue decisions without leaving their schema-28 keys active.
- Suppression affects actionable Queue projection and future sampling only. The original action and sampled-audit evidence remain in the immutable diagnostic history.
- Exact semantic siblings collapse to one Queue decision with related action IDs/counts; resolving that issue prevents the same problem from resurfacing under another generated action ID.
