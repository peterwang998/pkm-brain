...
**Status:** canonical living feature spec; the read-only Chief-of-Staff trial and independently scheduled Gmail mirror are locally release-verified and testing-ready, while fresh Gmail mailbox validation, owner review, and promotion remain pending
**Last verified:** 2026-07-14 with Ruff and diff checks green, 823 Python tests, 28 Swift tests, a signed local app installed with healthy runtime `0.1.5-762bf645-c1e72155`, and live scheduler/startup/budget-gate verification
...
The 2026-07-14 local release gate is complete: Ruff and diff checks are green, 823 Python tests and 28 Swift tests pass, and the signed local app bundle builds, installs, launches, and serves healthy runtime `0.1.5-762bf645-c1e72155`. The installed Gmail job registered on its dedicated 600-second lane and ran at startup, then stopped before a provider request because the pre-existing daily allowance was already `1200/1200`. That makes the workflow testing-ready and validates fail-closed scheduling/budget behavior, but not mailbox bootstrap or history checkpoints. Two isolated UI-acceptance attempts executed no app test bodies because macOS timed out enabling XCTest automation mode, so observed native UI acceptance remains a host-level final gate. Fresh Gmail mailbox validation, private-source quality review, daily briefing trust, and every empirical promotion gate remain pending.
