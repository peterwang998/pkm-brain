...
- `gmail_temporal_review.py`: pure message-level projection of supported citations, uncertainty sidecars, split-semantic reviews, alternatives, and reschedule structure; every output remains non-routable.
- `gmail_temporal_event_identity.py`: pure bounded event-unit identity bridge with revision-stable content anchors, exact authority-bound comparison plans, three complete external verdict sets, and unanimous clique-safe projection into provenance-retaining lifecycle assertions; it performs no provider call or persistence and every output remains deferred and non-routable.
- `gmail_temporal_thread_lifecycle.py`: pure cross-message review projection over one immutable Gmail thread snapshot. It preserves every scheduled occurrence and lifecycle source, derives reschedule/cancellation/completion state only when exact artifact hypotheses carry the same explicitly owner- or externally-verified stable event identity, and keeps missing, unverified, conflicting, or implicit identity/state changes as unresolved alternatives. Thread membership or a matching date never establishes event identity, and every derived view remains deferred and non-routable.
