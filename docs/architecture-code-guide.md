...
**Last verified:** 2026-07-20 against the unpromoted temporal-cognition branch based on rollback commit `d5405b9`; the Brain v2 target reaches schema 24 while the installed release remains on schema 21
...
- `gmail_temporal_leads.py`: content-free, snapshot-bound inventories of temporal expressions and event/action/lifecycle mentions plus bounded review-only association hints. Expressions preserve calendar dates, local wall times, ambiguity, blockers, and exact source offsets without pretending an unzoned time is an instant. Admission gates association hints but never changes either inventory. Oversized hint graphs retain the lossless inventories but expose omitted endpoint counts and mark candidate-edge counts as lower bounds.
- `gmail_temporal_selection.py`: the endpoint-only semantic authority boundary. An external selector may cite an expression, an event/action subject, an optional lifecycle cue, and an optional matching lead; deterministic code derives normalization, relation, planned/actual kind, lifecycle, confidence, blockers, and deferral. Snapshot fingerprints and content-bound IDs reject stale responses, and every result remains non-routable.
