...
**Status:** draft
**Last verified:** 2026-06-26 against uncommitted working tree atop commit `73b1e90`
...
- Implement or clearly gate topology mutations. `page_merge`, `page_split`, and `rename_page` are proposable action types, but application is declared unimplemented in `ACTION_TYPE_SPECS`.
- Add an active producer for derived page syntheses or keep the feature explicitly dormant. Storage, action application, staleness checks, and rendering exist for `wiki_page_syntheses`, but this code scan did not find a generator that creates synthesis Markdown from facts.
...
- Generated by: setup/sync CLI flows and direct config writes; `BrainPaths.config_file_for_read()` handles legacy config fallback.
...
- Deterministic vs LLM: BM25 FTS is deterministic. Default embeddings are deterministic hash embeddings because `BrainService` defaults `prefer_model_embeddings=False`. If model embeddings are explicitly preferred, a local sentence-transformer model may be used; that is model-assisted vectorization, not an external LLM agent call.
...
- Generated by: `synthesize_page` CoS actions through `apply_synthesize_page()`. In the current code scan, storage and action application exist, but there is no active generator path that creates synthesis Markdown from facts.
...
- Deterministic vs LLM: storing/staleness checks/rendering are deterministic. Any future synthesis text would likely be LLM-produced, but the current code does not show an active LLM synthesis producer.
