...
- `gmail_temporal_persistence.py`: source-bound append-only review runs, artifacts, execution/component receipts, idempotent compare-and-swap current heads, stale detection, atomic clear, and rollback. Production heads without a matching complete execution receipt are retired during migration and fail closed on reads. Review projection v2 also makes any persisted v1 head stale while retaining it as immutable compare-and-swap history; rollback cannot resurrect the retired schema. Execution evidence is a validated in-process capability and provider execution remains self-reported, not cryptographically attested; a malicious caller already executing inside the Brain process can fabricate that capability. It cannot write facts, events, reminders, or operational state.
