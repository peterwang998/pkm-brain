...
- `scripts/build_gmail_temporal_public_challenge_v3.py` and `scripts/run_gmail_temporal_public_challenge.py`: freeze and execute a public-synthetic, non-release semantic challenge without private Gmail. The freezer requires fresh disjoint owner-only roots, supported and uncertainty calibration, structured forbidden bindings, a non-vacuous canonical-title denominator, and deterministic frontier authority for every gold member. The runner seals case-atomic three-run predictions before opening gold, authenticates exact prior-launcher artifacts for historical rescoring, and reports effective recall, confirmed recall, supported/review precision, complete-group recall, canonical-title recall, negative selection, and forbidden hypotheses separately. Legacy gold retains its original semantics; replay never rewrites an old failure.
