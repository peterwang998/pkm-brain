...
- `scripts/build_gmail_temporal_public_challenge_v3.py` and `scripts/run_gmail_temporal_public_challenge.py`: freeze and execute a public-synthetic, non-release semantic challenge without private Gmail. The freezer requires fresh disjoint owner-only roots, supported and uncertainty calibration, structured forbidden bindings, a non-vacuous canonical-title denominator, and deterministic frontier authority for every gold member. The runner seals case-atomic three-run predictions before opening gold and authenticates exact prior-launcher artifacts for historical rescoring. Score v10 treats an exact temporal binding as the same semantic answer whether the verifier calls it `supported` or `uncertain`; effective recall, artifact precision, complete-group recall, and canonical-title recall therefore measure extraction and structure, while confirmed recall and supported precision measure confidence calibration. Matching prefers a calibration-consistent duplicate deterministically, calibration-only duplicate gold is invalid, and unlabeled cluster-review escalations are reported as unscored workload instead of false-positive artifacts. Negative selection and forbidden bindings remain separate hard gates. Legacy gold remains immutable; a replay never rewrites an old label or prediction.
