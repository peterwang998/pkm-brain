...
- **Current fact-quality blocker added 2026-07-06:** H1c prompt hardening plus resolver no-op suppression removed the bogus `fact_supersede` audit failures, but the second 5-doc sample still had 7/33 audited fact-upserts marked bad under extractor `low` effort. Live extractor effort is now back to `medium`; rerun the same sample shape before any T4.1 apply.
