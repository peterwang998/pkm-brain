...
**Last verified:** 2026-07-06 against commit `bd8c26c` plus the narrowed-critic temp rerun.
...
- **Current fact-quality gate updated 2026-07-06:** H1c prompt hardening plus resolver no-op suppression removed the bogus `fact_supersede` audit failures. The low-effort rerun still had 7/33 audited fact-upserts marked bad; the medium-effort rerun improved to 4/34 but still exceeded the policy demotion threshold. Live extractor effort is now `medium` and live policy v4 critic-gates clean new fact-upserts. The v4 temp samples passed post-critic audit on all applied fact-upserts. The remaining blocker is no longer applied-fact quality; it is critic scale/residue. The first v4 run blocked 21/39 clean candidates; manual audit showed a mix of real overclaims and over-conservative blocking. The narrowed critic rerun applied 20, blocked 18, and left 3 precheck/conflict residues; 20/20 applied actions audited ok. Critic rationales now persist on actions for auditability. Before rebuild scale, decide whether blocked clean facts become auditable rejects in rebuild mode and add critic runtime guard/parallelism.
