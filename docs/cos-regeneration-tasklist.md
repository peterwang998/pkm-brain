...
**Last verified:** 2026-07-05 against the working tree after `60e575e` plus the current regeneration dry-run diff.
...
- **Gardener caveat updated 2026-07-05:** the old xhigh 600s megacall timeout is no longer the active blocker. G1/G1.2 now use per-candidate judgment, timeout isolation, and effort tiering (`tests/test_gardener.py` covers timeout isolation, auditable drops, and effort tiers). Remaining work is operational: run the full shadow gardener pass with configured roles and verify timeout/error counts before T4.1.
- **Extraction autonomy caveat added 2026-07-05:** promoted clean `fact_upsert` rules are wired, but they correctly fail closed until a passing labeled extraction eval report exists. Live dry-run reports `extraction_eval_gate.ready=false` because `/Users/Peter/brain/evals/extraction_labels.jsonl` has 0 cases and the latest extraction report is unlabeled. This blocks autonomous fact writes even when extractor output is clean.
...
- [ ] **T1.2 — Run authoring on a 5–10 doc sample** (meetings/notes only) via `brain cos run` or scoped extraction. Inspect created facts.
  - *Result 2026-07-05:* real-model 5-doc temp sample extracted clean candidates but did not create LLM facts under the default promoted path. Non-promoted sample accepted 40/46 raw facts in 64s with 92.5% canonical routing and 3 fallback/unrouted facts; promoted-policy sample matched `policy_v2_fact_upsert_clean_l2` but every clean action was downgraded to L3 by `eval_gate_failed` because the extraction eval report is unlabeled. This is the expected blocker, not an extractor failure.
...
- [ ] **T1.4 — Run the eval bundle**; confirm gates are real and green: extraction span-coverage (scoped to `extraction_method='llm'`), topology **real F1** (not smoke), conflict `false_truth_resolutions=0`, retrieval, routing.
  - *Result 2026-07-05:* retrieval was rerun after the live sentence-transformer flip and passed (`eval_c3d362b0548340c0`). Extraction remains blocked for autonomy because the gate requires labeled fixtures, not just vacuous legacy-excluded span coverage.
