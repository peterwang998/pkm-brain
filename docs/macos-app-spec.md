...
- `GET /api/scheduler` → jobs with `{id, enabled, cadence_s, last_run_at, last_status, last_error, last_result, next_due_at, running}`, plus `paused_until`. Skipped jobs must preserve the no-op reason in `last_error`/`last_result.reason` so Ops never shows an unexplained "skipped".
...
- **2026-07-08 M3 secondary Mac mini migration:** staged M3 source/app at `/Users/peterwang/pkm-brain-m3-6f4ac0d` on `Peters-Mac-mini.local`; created runtime backup `/Users/peterwang/Library/Application Support/PKM Brain/migration/runtime-backups/20260708T203450Z`; retired `com.pkm-brain.capture-secondary` to the app migration plist backup with rollback script; installed app-managed shims; launched the app-managed daemon from runtime `0.1.0-762bf645-42147f2b`; migration plan now reports `adopt` with no detected pkm-brain LaunchAgents. Secondary scheduler exposed `secondary_tick` + `nightly`, ran `secondary_tick` successfully at `2026-07-08T20:35:44+00:00`, Codex MCP was rewritten to `/Users/peterwang/Library/Application Support/PKM Brain/bin/brain-mcp`, and SMAppService login-item status was enabled via the app one-shot command.
- **2026-07-08 M3 two-machine verification after secondary migration:** primary app daemon ran `sync:Peters-Mac-mini` at `2026-07-08T20:37:05+00:00` with status `ok`, `files_pulled=37`, `files_pushed=976`, and `remote_ingest_status=ok`; `brain sync status` outside the sandbox reported matching manifest hash `659ae5b786e220104e3175fca05b6247be7a9633834e8c4a8a3772ff4e8d7f46`, `mirror_current=true`, and no warnings.
- **2026-07-08 M4 native Queue implementation (`a81e8cf`, `e8eebed`, `0c14ff2`):** added SwiftUI Queue over `/api/queue` with split list/detail, focused keyboard handler, optimistic decisions, undo countdown, conflict evidence side-by-side, batch selection, and queue/daemon/nightly/connector attention notifications. Automated temp-home acceptance `test_m4_queue_acceptance_mixed_decisions_land_and_undo` seeds 20 mixed items (conflicts, unrouted facts, memories, topology actions), verifies side-by-side conflict evidence, dispatches decisions through `/api/queue/{id}/decision`, exercises undo via `/api/queue/undo`, and checks final `open_questions`, `cos_actions`, `memories`, routed facts, and contracts against SQLite ground truth. Gates passed: `swift test --package-path app` (9 tests), `scripts/build-app.sh`, `uv run ruff check .`, and `uv run pytest -q` (391 tests). Remaining before strict M4 sign-off: an observed GUI keyboard-only 20-item triage pass in the signed app.
- **2026-07-09 M4 live polish:** Today pulse chips now expose detail text (audit warning counts, nightly run timestamp, index/sync/job context); Ops preserves scheduler no-op reasons (`nightly` skipped because the last successful run was less than 20h old); adopted homes with no legacy LaunchAgents hide the migration shim banner; native Queue now loads 50-card pages with Load More; Codex-backed policy calls pass `--skip-git-repo-check` for daemon-owned noninteractive runs. Live primary verification after relaunch: `/api/queue?limit=50` returned 50 of 701 items in 0.05s with `next_cursor=50`, and a forced nightly due-check populated Ops with `last_error="last successful nightly run is less than 20 hours old"`. Gates passed: `swift test`, `uv run ruff check .`, `uv run pytest -q` (392 tests), and `scripts/build-app.sh`.
