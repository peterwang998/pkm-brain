...
**Status:** implemented — v2 shell, Today, Queue, Wiki, Entities, Ask, Ops, static asset serving, and legacy `ui_shell()` retirement are in the working tree ready to commit.
**Last verified:** 2026-07-07 against commit `c736f11` plus the UI v2 working tree; `uv run pytest tests/test_ui_auth.py tests/test_ui_endpoints.py -q` passed (24 tests).
