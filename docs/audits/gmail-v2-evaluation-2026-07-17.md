...
The first pilot's pre-remediation local Codex history remains pending cleanup. A content-free inventory found 229 isolated one-shot sessions from that interval; 182 contained recognized Gmail-sensitive values. The primary working thread is not in that set. Deleting those supported session records is destructive and requires explicit owner authorization, so no cleanup was performed as part of this evaluation.
...
- Pattern-based secret scanning cannot prove the absence of all private or sensitive content. The old external Codex sessions remain until the owner authorizes cleanup.
