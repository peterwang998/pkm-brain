...
**Status:** T0-T5 base temporal cognition is implemented and promoted in Brain `0.2.4`: original-Brain fact parity, universal source/knowledge clocks, optional predicate validity, event entities with source-backed occurrence facts, bitemporal retrieval, and the bounded local Gmail Knowledge projection are live-compatible. Gmail external extraction and temporal hypothesis review remain disabled by default and evaluation/review-only; they are not promoted into trusted facts, reminders, or actions.
**Last verified:** 2026-07-29 against release-line commit `2085ec3` and Knowledge schema 26. The historical development and gate evidence below remains the rationale for the promoted base architecture; later production deployment, Gmail recall, parity, and lookup-resilience evidence is recorded in the linked runbooks and audits.
