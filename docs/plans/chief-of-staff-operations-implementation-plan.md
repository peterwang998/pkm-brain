...
| COS-6 | reversible cross-source episodes, satisfaction, full meeting-packet dependency manifests, and production briefing gates | none | gated |
| COS-7 | local versioned drafts, active reminder state/delivery ledgers, and guarded action plans | none | gated |
...
The richer proactive-flow substrate is future work, not part of the release-verified tranche. COS-6 must replace revision-only meeting-packet validity with a manifest over every operational and Brain dependency. COS-7 must add non-indexed `ops_drafts`/version history plus durable reminder rules, occurrence state, delivery attempts/receipts, timezone and restart semantics, snooze/acknowledgement, and cancellation/reschedule handling. None of those tables or lifecycle contracts is implemented today.
