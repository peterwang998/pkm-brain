...
- Ingest and query embedding share one object: `BrainService.embedding_provider` feeds chunk upserts/rebuilds and query embedding. Provider consistency is structural once resolution is config-driven.
- Rebuild machinery exists: `rebuild_vector_index()` (timestamped backup + verification), `rebuild_vector_index(missing_only=True)`, `reset_retrieval_index()`, `index doctor` with `vector_chunk_ids()` for SQLite↔LanceDB consistency.
...
Provider selection moved from the deleted `prefer_model_embeddings` constructor flag to config resolved once per process:
...
- Delete the `prefer_model_embeddings` parameter and update all construction sites.
