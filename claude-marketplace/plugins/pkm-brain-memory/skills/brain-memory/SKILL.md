...
This skill is an activation policy. The actual memory access should happen through the `pkm-brain` MCP tools. Do not read Brain's SQLite database or LanceDB index directly unless the user explicitly asks for low-level debugging.
...
Prefer MCP when available because it gives Claude Code structured tool results without shell parsing.
