...
struct PKMBrainKitTests {
    ...
    func schedulerFixtureDecodes() throws {
        ...
        let archive = try #require(scheduler.jobs.first { $0.id == "gmail_archive_sync" })
        #expect(archive.displayName == "Copy Gmail history securely")
        #expect(archive.displayStatus == "partial")
        #expect(archive.statusDetail?.contains("1,750 messages stored") == true)
        let knowledge = try #require(scheduler.jobs.first { $0.id == "gmail_knowledge_ingest" })
        #expect(knowledge.displayName == "Index important Gmail knowledge")
        #expect(knowledge.displayStatus == "complete")
